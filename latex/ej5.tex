...
iteraciones. El ciclo interior recorre el conjunto $U \subseteq G_2$ de los
...
se itera a su vez, para cada nodo $u \in U$, sobre el conjunto de sus vecinos,
cuyo cardinal es $\deg(u)$. Para cada uno de estos vecinos se verifica si
pertenece al mapeo y, en caso afirmativo, debe decidirse si son adyacentes en
$G_1$ los nodos mapeados a $u$ y su vecino en cuestión. Esto último tiene
complejidad $\ord(\deg(u'))$, siendo $u'$ el nodo de $G_1$ mapeado a $u$, que
puede acotarse por $\ord(N_1)$. De todo lo anterior se concluye que la
complejidad del primer ciclo es:

\begin{align*}
\ord\left( N_1 \times \left( \sum_{u \in U} \left(1 + \deg(u) \times N_1
\right) \right) \right)
&= \ord \left( N_1 \times \left( (N_2 - N_1) + N_1 \times \sum_{u \in U} \deg
(u) \right) \right) \\
&= \ord \left( N_1 \times \left( (N_2 - N_1) + N_1 \times \sum_{v \in V(G_2)}
\deg(v) \right) \right) \\
...
almacena los nodos que no pertenecen al mapeo. Por últmo se incorporan, cada
